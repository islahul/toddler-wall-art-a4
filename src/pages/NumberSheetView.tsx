...
import NumberFlashcardSheet from '@/components/NumberFlashcardSheet'
import { NumberFlashcard } from '@/types/flashcard'
import { Button } from '@/components/ui/button'

export const NumberSheetView: React.FC = () => {
  const router = useRouter()
  const { sheetNumber } = useParams({ from: '/number-sheet/$sheetNumber' })
  const [currentSheet, setCurrentSheet] = useState<number>(0)
  const [allSheets, setAllSheets] = useState<NumberFlashcard[][]>([])

  useEffect(() => {
    const sheets = generateNumberFlashcardData()
    setAllSheets(sheets)
  ...
  const navigateSheet = (offset: number) => {
    ...
    if (newSheetIndex >= 0 && newSheetIndex < allSheets.length) {
      ...
      router.navigate({ to: '/number-sheet/$sheetNumber', params: { sheetNumber: (newSheetIndex + 1).toString() } })
    }
  }
  ...
  const numbers = currentSheetData?.map((card: NumberFlashcard) => card.number).join(' - ') || ''
      ...
      <div className="max-w-7xl mx-auto">
        <header className="mb-8 print:hidden">
          <Button asChild variant="link" className="px-0">
            <Link to="/number-project/$projectId" params={{ projectId: 'numbers-cats' }}>
          ...
          <div className="text-center mt-4">
            ...
            <p className="text-lg text-muted-foreground mt-2 font-mono">Numbers: {numbers}</p>
          </div>
        </header>
